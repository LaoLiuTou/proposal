    ...
    var index = {
        ...
        initLeftNav: function (menus, id) {
            ...
            if (id) {
                ...
                $.each(menus, function (i, m) {
                    ...
                    html += '<li class="layui-nav-item' + cls + '" lay-unselect>' +
                        '<a ' + aClass + ' data-id="' + m.id + '" href="' + uri + '" '+(m.uri?"target=\"_blank\"":"")+'>' + m.name + '</a>' +
                        '</li>';
                });
                $('#topMenu').append(html);
            }
            ...
            $('.layui-layout-admin .layui-side').load('components/side.html', function () {
                var _menu = [];
                firstMenus.forEach(function(item){
                    if (''+item.hide === '0') _menu.push(item);
                });
                laytpl(sideNav.innerHTML).render(_menu, function (html) {
                    $('#sideNav').after(html);
                });
                element.render('nav');
