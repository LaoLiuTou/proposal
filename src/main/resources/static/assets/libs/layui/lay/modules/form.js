        ...
        s = "layui-this", o = "layui-hide", c = "layui-disabled", u = function () {
            this.config = {
                verify: {
                    ...
                    ////lt start////
                    jdc_petitionerName: [/[\S]+/, "请输入姓名"],
                    jdc_idcard: [/(^\d{15}$)|(^\d{17}(x|X|\d)$)/, "请输入正确的身份证号"],
                    jdc_brthday: [/^(\d{4})[-\/](\d{1}|0\d{1}|1[0-2])([-\/](\d{1}|0\d{1}|[1-2][0-9]|3[0-1]))*$/, "请选择出生年月"],
                    jdc_permanentAddress: [/[\S]+/, "请输入户籍地址"],
                    jdc_currentAddress: [/[\S]+/, "请输入现居住地"],
                    jdc_visitDate: [/^(\d{4})[-\/](\d{1}|0\d{1}|1[0-2])([-\/](\d{1}|0\d{1}|[1-2][0-9]|3[0-1]))*$/, "请选择来访日期"],
                    jdc_name: [/[\S]+/, "请输入事件名称"],
                    jdc_contentTypeIdStr: [/[\S]+/, "请选择内容分类"],
                    jdc_comeNum: function (e) {
                        if (!e || isNaN(e)) return "来访人数只能填写数字"
                    },
                    jdc_receptionId: [/[\S]+/, "请选择接访人"],
                    ////lt end////
                }
            }
        };
